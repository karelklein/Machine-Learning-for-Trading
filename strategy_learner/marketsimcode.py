...
def format_trades(trades):
    # takes in a trades df from StrategyLearner
    orders = trades.copy()
    orders['Order'] = 0
    orders['Shares'] = 0
    ...
    for i in range(trades.shape[0]):
        orders.ix[i,'Symbol'] = symbol
        if trades.ix[i,symbol] > 0:
            orders.ix[i,'Order'] = 'BUY'
        elif trades.ix[i,symbol] < 0:
            orders.ix[i,'Order'] = 'SELL'
        else:
            orders.ix[i,'Order'] = 'BUY'
        orders.ix[i,'Shares'] = abs(trades.ix[i,symbol])
    return orders
...
def compute_portvals(orders, start_val=1000000, commission=9.95, impact=0.005):
    ...
    # convert orders from StrategyLearner into proper format

    if orders.shape[1] < 2:
        orders = format_trades(orders)
